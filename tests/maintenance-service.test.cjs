...
test('Meeting search returns mapped display names', () => {
  const result=ksp.kspSearchMeetingRecords(createFakeEnvironment(),{gpId:'GP-000002'});
...
test('Meeting update preserves ID/Doc, increments Version, updates Doc, and audits metadata only', () => {
  ...
  const result=ksp.kspUpdateMeetingMaintenance(env,{meetingId:'MTG-000001',expectedVersion:1,date:'2026-08-03',time:'10:00',locationId:'OPT-LOC-001',gpId:'GP-000002',assetClassId:'OPT-AC-002',capitalTypeId:'OPT-CT-001',counterparty:'Person',internalParticipants:'Team',notes:'secret\nnotes'});
...
test('stale Meeting update is rejected without mutating the document', () => {
  ...
  const result=ksp.kspUpdateMeetingMaintenance(env,{meetingId:'MTG-000001',expectedVersion:99,date:'2026-08-03',gpId:'GP-000002',assetClassId:'OPT-AC-002',notes:'new'});
...
test('failed Meeting commit restores Doc while claim is still owned', () => {
  ...
  const result=ksp.kspUpdateMeetingMaintenance(env,{meetingId:'MTG-000001',expectedVersion:1,date:'2026-08-03',gpId:'GP-000002',assetClassId:'OPT-AC-002',notes:'new'});
...
test('Meeting status update rejects stale version and otherwise increments Version', () => {
  const env=createFakeEnvironment();
  const ok=ksp.kspChangeMeetingStatus(env,{meetingId:'MTG-000001',expectedVersion:1,targetStatus:'Inactive'});
  assert.equal(ok.ok,true); assert.equal(ok.record.status,'Inactive'); assert.equal(ok.record.version,2);
  const stale=ksp.kspChangeMeetingStatus(env,{meetingId:'MTG-000001',expectedVersion:1,targetStatus:'Active'});
...
test('Pitchbook context move allocates destination next sequence and preserves File ID', () => {
  const env=createFakeEnvironment();
  const result=ksp.kspUpdatePitchbookMaintenance(env,{documentId:'DOC-000001',expectedUpdatedAt:'2026-08-01T00:00:00.000Z',date:'2026-08-02',gpId:'GP-000001',assetClassId:'OPT-AC-001',capitalTypeId:''});
...
test('stale Pitchbook metadata update is rejected without renaming file', () => {
  ...
  const result=ksp.kspUpdatePitchbookMaintenance(env,{documentId:'DOC-000001',expectedUpdatedAt:'stale',date:'2026-08-02',gpId:'GP-000001',assetClassId:'OPT-AC-001',capitalTypeId:''});
...
test('Pitchbook status cannot reactivate a row without authoritative file', () => {
  ...
  const result=ksp.kspChangePitchbookStatus(env,{documentId:'DOC-000003',expectedUpdatedAt:'t',targetStatus:'Active'});
...
test('quick-add GP returns an existing normalized duplicate instead of adding a row', () => {
  ...
  const result=ksp.kspQuickAddGp(env,'  ＡＰＯＬＬＯ ');
...
test('Master add, rename, reorder, deactivate and reactivate are supported', () => {
  const env=createFakeEnvironment();
  const add=ksp.kspMutateMaster(env,{entity:'OPTION',action:'ADD',type:'ASSET_CLASS',name:'VC'}); assert.equal(add.ok,true);
  const id=add.record.Option_ID;
  const rename=ksp.kspMutateMaster(env,{entity:'OPTION',action:'RENAME',id,name:'Venture Capital'}); assert.equal(rename.record.Name,'Venture Capital');
  const reorder=ksp.kspMutateMaster(env,{entity:'OPTION',action:'REORDER',id,sortOrder:1}); assert.equal(reorder.record.Sort_Order,1);
  const off=ksp.kspMutateMaster(env,{entity:'OPTION',action:'DEACTIVATE',id}); assert.equal(off.record.Status,'Inactive');
  const on=ksp.kspMutateMaster(env,{entity:'OPTION',action:'REACTIVATE',id}); assert.equal(on.record.Status,'Active');
});
...
test('Actor and audit failures are non-blocking to a successful Meeting update', () => {
  ...
  const result=ksp.kspUpdateMeetingMaintenance(env,{meetingId:'MTG-000001',expectedVersion:1,date:'2026-08-03',gpId:'GP-000002',assetClassId:'OPT-AC-002',notes:''});
...
test('audit retention cleanup removes old rows and writes a cleanup event', () => {
  ...
  const result=ksp.kspRunAuditRetentionCleanup(env);
...
test('historical records keep display names after a Master becomes Inactive', () => {
  const maps = ksp.kspBuildAllMasterMaps(
    [{ GP_ID:'GP-9', GP_Name:'Former GP', Status:'Inactive' }],
    [{ Option_ID:'OPT-AC-9', Type:'ASSET_CLASS', Name:'Former Class', Status:'Inactive' }]
  );
  const record = ksp.kspMapMeetingSearchResult({ Meeting_ID:'MTG-000009', GP_ID:'GP-9', Asset_Class_ID:'OPT-AC-9' }, maps);
...
test('Option reorder audit snapshot captures every affected option order', () => {
  ...
  const snapshot=ksp.kspOptionOrderAuditSnapshot(rows);
...
test('explicit invalid search limits are rejected rather than silently reset', () => {
  assert.throws(() => ksp.kspValidateRecordSearch(ksp.kspNormalizeRecordSearch({ limit: -1 })), /検索件数上限/);
  assert.throws(() => ksp.kspValidateRecordSearch(ksp.kspNormalizeRecordSearch({ limit: 9999 })), /検索件数上限/);
});

test('Phase 1 diagnostics are non-destructive and report resource/schema/capability health', () => {
  const env=createFakeEnvironment();
  const result=ksp.kspGetPhase1Diagnostics(env);
