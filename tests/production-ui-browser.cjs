...
async function captureMeetingLayout(page,width,height,label,out){
  ...
  if(width>720){
    ...
    assert.equal(evidence.sidebar.motifDisplay,'block');assert.equal(evidence.sidebar.motifOpacity,'0.72');assert.notEqual(evidence.sidebar.activeShadow,'none');assert.notEqual(evidence.sidebar.iconFilter,'none');
  }else{
