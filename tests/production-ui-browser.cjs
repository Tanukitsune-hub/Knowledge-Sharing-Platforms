...
const types=[{code:'GP',label:'GP / 運用会社'},{code:'LP_ASSET_OWNER',label:'LP / Asset Owner',optionType:'COUNTERPARTY_LP'},{code:'NISSAY_INTERNAL',label:'日本生命'},{code:'GROUP_COMPANY',label:'グループ会社'},{code:'CONSULTANT_GATEKEEPER',label:'Consultant / Gatekeeper'},{code:'OTHER',label:'その他'}];
...
const quickCounterparty={id:'CP-QUICK',type:'NISSAY_INTERNAL',name:'Synthetic Modal Counterparty',status:'Active'};
...
const fixtures={
  ...
  quickAddCounterparty:{ok:true,counterparty:quickCounterparty,record:{Counterparty_ID:quickCounterparty.id,Counterparty_Type:quickCounterparty.type,Counterparty_Name:quickCounterparty.name,Status:quickCounterparty.status},masters:{counterparties:[...counterparties,quickCounterparty],options:masterOptions}},
  ...
  getAiProviderAdminData:{ok:true,canMutate:true,openai:{keyConfigured:false,vectorStoreReady:false,status:'DISABLED'},gemini:{keyConfigured:false,storeReady:false,status:'DISABLED'},modelPolicyPersisted:false,modelPolicy:{profiles:[]}},
  ...
  getMeetingActivityAnalytics:{ok:true,filterOptions:{counterpartyEntities:[],assetClasses:[],teams:[],meetingTypes:[],statuses:[]},period:{mode:'monthly',startDate:'2025-09-19',endDate:'2026-09-19',bucketCount:13},headline:{meetingCount:0,activeMeetingCount:0,distinctCounterpartyCount:0,openFollowUpCount:0},series:[],breakdown:{items:[],totalCount:0,omittedCount:0},drill:{records:[],omittedCount:0},adminCheckAvailable:false}
};
...
async function captureCrossTabLayout(page,width,height,label){
  await page.setViewportSize({width,height});
  const targets={knowledge:'.knowledge-period-row',meeting:'#meeting-form>.grid','meeting-past':'.meeting-past-filter-grid','entity-workspace':'.entity-workspace-selector-grid','activity-analytics':'.analytics-controls',masters:'.master-layout','ai-provider-settings':'#page-ai-provider-settings .filter-grid'};
  const result={};
  for(const[name,selector]of Object.entries(targets)){
    await page.locator('#nav-'+name).click();
    const observation=await page.locator(selector).first().evaluate(node=>{const template=getComputedStyle(node).gridTemplateColumns;return{columns:template.split(' ').length,columnTemplate:template,overflow:document.documentElement.scrollWidth>innerWidth,pageVisible:node.closest('.page').classList.contains('active')}});
    assert.equal(observation.pageVisible,true,label+' '+name+' active');assert.equal(observation.overflow,false,label+' '+name+' overflow');if(name!=='masters')assert.equal(observation.columns,width<=720?1:12,label+' '+name+' columns '+observation.columnTemplate);result[name]=observation;
  }
  return result;
}
async function main(){
  ...
  try{
    ...
    assert.equal(await page.locator('#counterparty-modal-backdrop').isVisible(),true);assert.equal(await page.locator('#counterparty-modal').getAttribute('aria-modal'),'true');assert.equal(await page.locator('#counterparty-modal-type option').count(),7);assert.equal(await page.evaluate(()=>document.activeElement.id),'counterparty-modal-type');await page.screenshot({path:path.join(out,'05-counterparty-modal.png'),fullPage:false});
    await page.locator('#counterparty-modal-submit').click();assert.match(await page.locator('#counterparty-modal-status').textContent(),/面談先種別/);assert.equal(calls.filter(call=>call.name==='quickAddCounterparty').length,quickBefore);
    await page.locator('#counterparty-modal-type').selectOption('NISSAY_INTERNAL');await page.locator('#counterparty-modal-submit').click();assert.match(await page.locator('#counterparty-modal-status').textContent(),/面談先名/);assert.equal(calls.filter(call=>call.name==='quickAddCounterparty').length,quickBefore);
    await page.locator('#counterparty-modal-name').fill('Synthetic Modal Counterparty');await page.locator('#counterparty-modal-submit').click();await page.waitForFunction(()=>document.getElementById('meeting-counterpartyId').value==='CP-QUICK');assert.equal(await page.locator('#counterparty-modal-backdrop').isVisible(),false);assert.equal(calls.filter(call=>call.name==='quickAddCounterparty').length,quickBefore+1);assert.equal(calls.find(call=>call.name==='quickAddCounterparty').payload.type,'NISSAY_INTERNAL');checks.push('shared Counterparty modal / required fields / non-default type / Meeting auto-select');
    ...
    const pastLayout=await page.evaluate(()=>{const read=id=>{const node=document.querySelector(id),style=getComputedStyle(node);return{columnStart:style.gridColumnStart,columnEnd:style.gridColumnEnd,rowStart:style.gridRowStart}};return{columns:getComputedStyle(document.querySelector('.meeting-past-filter-grid')).gridTemplateColumns.split(' ').length,dateFrom:read('.meeting-past-date-from-field'),dateTo:read('.meeting-past-date-to-field'),counterparty:read('.meeting-past-counterparty-field'),asset:read('.meeting-past-asset-field'),team:read('.meeting-past-team-field'),types:read('.meeting-past-types-field')}});
    assert.equal(pastLayout.columns,12);assert.deepEqual(pastLayout.dateFrom,{columnStart:'1',columnEnd:'span 2',rowStart:'2'});assert.deepEqual(pastLayout.dateTo,{columnStart:'3',columnEnd:'span 2',rowStart:'2'});assert.deepEqual(pastLayout.counterparty,{columnStart:'1',columnEnd:'span 4',rowStart:'3'});assert.deepEqual(pastLayout.asset,{columnStart:'5',columnEnd:'span 2',rowStart:'2'});assert.deepEqual(pastLayout.team,{columnStart:'7',columnEnd:'span 2',rowStart:'2'});assert.deepEqual(pastLayout.types,{columnStart:'5',columnEnd:'span 6',rowStart:'3'});assert.equal(await page.locator('#meeting-past-fundStrategy').isVisible(),false);assert.equal(await page.locator('#meeting-past-filterStatus').isVisible(),false);
    const beforeFilterCalls=calls.filter(call=>call.name==='searchMeetingRecords').length;await page.locator('#meeting-past-search').click();await page.waitForTimeout(30);await page.locator('[data-meeting-type-filter="meeting-past"]').nth(0).check();await page.locator('#meeting-past-search').click();await page.waitForTimeout(30);await page.locator('[data-meeting-type-filter="meeting-past"]').nth(1).check();await page.locator('#meeting-past-search').click();await page.waitForFunction(()=>document.getElementById('meeting-past-status').textContent.includes('件を表示しました。'));await page.waitForTimeout(30);const filterCalls=calls.filter(call=>call.name==='searchMeetingRecords').slice(beforeFilterCalls);assert.deepEqual(filterCalls.map(call=>call.payload.meetingTypeCodes),[[],['ANNUAL_REVIEW'],['ANNUAL_REVIEW','OFFICE_VISIT']]);assert.deepEqual(filterCalls.map(call=>call.payload.meetingTypeCode),['','ANNUAL_REVIEW','']);assert.deepEqual(filterCalls.map(call=>[call.payload.capitalTypeId,call.payload.followUpOnly,call.payload.fundStrategy,call.payload.status]),[['',false,'','Active'],['',false,'','Active'],['',false,'','Active']]);await page.locator('#meeting-past-clear').click();assert.equal(await page.locator('#meeting-past-filterStatus').inputValue(),'Active');checks.push('Past Meetings compact layout / hidden fixed filters / reset Active / Meeting Type OR payload');
    ...
    await page.locator('#nav-knowledge').click();assert.equal(await page.locator('#knowledge-instruction').isEditable(),false);assert.equal(await page.locator('#knowledge-model-profile').inputValue(),'');
    ...
    assert.equal(await page.locator('#knowledge-instruction').inputValue(),'');assert.equal(await page.locator('#knowledge-model-profile').inputValue(),'');assert.equal(await page.locator('#knowledge-instruction-label').textContent(),'AI検索 指示入力欄');assert.equal(calls.some(x=>/startKnowledge|searchKnowledge/.test(x.name)),false);
    ...
    await page.locator('#nav-masters').click();await page.locator('[data-master-tab="ASSET_CLASS"]').click();assert.equal(await page.locator('#option-master-panel').isVisible(),true);assert.equal(await page.locator('#counterparty-master-panel').isVisible(),false);assert.equal(await page.locator('#option-add-type').count(),0);await page.locator('#masters-refresh').click();await page.waitForTimeout(30);assert.equal(await page.locator('[data-master-tab="ASSET_CLASS"]').getAttribute('aria-selected'),'true');await page.locator('[data-master-tab="COUNTERPARTY"]').click();await page.locator('#masters-add-counterparty').click();assert.equal(await page.locator('#counterparty-modal-backdrop').isVisible(),true);await page.locator('#counterparty-modal-cancel').click();assert.equal(await page.locator('#counterparty-modal-backdrop').isVisible(),false);assert.equal(await page.evaluate(()=>document.activeElement.id),'masters-add-counterparty');checks.push('Masters four tabs preserve selection / fixed option type / shared modal focus restore');
    await page.locator('#nav-ai-provider-settings').click();await page.waitForTimeout(30);assert.equal(await page.locator('[id^="shared-admin-"]').count(),0);assert.equal(await page.locator('#ai-provider-openai-enable').isDisabled(),false);checks.push('owner-only admin controls available without shared password gate');
