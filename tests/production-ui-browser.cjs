...
async function captureCrossTabLayout(page,width,height,label){
  await page.setViewportSize({width,height});
  const targets={knowledge:'.knowledge-primary-row',meeting:'#meeting-form>.grid','meeting-past':'.meeting-past-filter-grid','entity-workspace':'.entity-workspace-selector-grid','activity-analytics':'.analytics-controls',masters:'.master-layout','ai-provider-settings':'#page-ai-provider-settings .filter-grid'};
  const result={};
  for(const[name,selector]of Object.entries(targets)){
    ...
    assert.equal(observation.pageVisible,true,label+' '+name+' active');assert.equal(observation.overflow,false,label+' '+name+' overflow');assert.equal(observation.columns,width<=720?1:12,label+' '+name+' columns '+observation.columnTemplate);result[name]=observation;
  }
  return result;
}
async function main(){
