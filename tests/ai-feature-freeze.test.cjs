...
test('format registry contains exactly the six accepted formats and stable MIME mappings',()=>{
  assert.deepEqual(Array.from(ksp.kspGetAiFormatExtensions()),['pdf','pptx','xlsx','docx','txt','eml']);
  assert.equal(ksp.kspGetAiFormatDefinition('pdf').uploadMimeType,'application/pdf');
  assert.equal(ksp.kspGetAiFormatDefinition('pptx').readStrategy,'DIRECT_BINARY');
  assert.equal(ksp.kspGetAiFormatDefinition('eml').uploadMimeType,'text/plain');
});

test('source descriptor enforces MIME and 25MB product limit',()=>{
  assert.equal(ksp.kspValidateAiSourceDescriptor('pdf','application/pdf',12).extension,'pdf');
  assert.throws(()=>ksp.kspValidateAiSourceDescriptor('pdf','text/plain',12),/MIME/);
  assert.throws(()=>ksp.kspValidateAiSourceDescriptor('pdf','application/pdf',25*1024*1024+1),/25MB/);
});
...
for(const [extension,source] of Object.entries(formatCases)){
  test(`${extension} source dispatch preserves authoritative bytes and explicit upload MIME`,()=>{
    ...
    const maps=ksp.kspBuildAiMasterMaps(c.gpRows,c.optionRows);
    const built=plain(ksp.kspBuildFeatureFreezeAiSource(env,{sourceType:'Pitchbook',sourceId:'DOC-000001',row:c.pitchbookRows[0]},maps));
    assert.equal(built.payloadKind,'binary');
    assert.deepEqual(built.bytes,source.bytes);
    assert.equal(built.mimeType,ksp.kspGetAiFormatDefinition(extension).uploadMimeType);
    assert.equal(built.byteLength,source.bytes.length);
  });
}

test('EML normalizes encoded headers and quoted-printable plain body',()=>{
  const text=ksp.kspNormalizeEmlText(fixture('plain-quoted-printable.eml'));
...
test('EML uses HTML fallback and excludes attachment, style, and script content',()=>{
  const text=ksp.kspNormalizeEmlText(fixture('html-with-attachment.eml'));
...
test('EML decodes base64 UTF-8 body',()=>{
  assert.match(ksp.kspNormalizeEmlText(fixture('base64-utf8.eml')),/こんにちは、APACの更新です。/);
});

test('malformed and attachment-only EML fail deterministically',()=>{
  assert.throws(()=>ksp.kspNormalizeEmlText('Subject: no separator'),/separator/);
  const attachmentOnly='Subject: x\nMIME-Version: 1.0\nContent-Type: multipart/mixed; boundary="b"\n\n--b\nContent-Type: text/plain; name="x.txt"\nContent-Disposition: attachment; filename="x.txt"\n\nsecret\n--b--';
  assert.throws(()=>ksp.kspNormalizeEmlText(attachmentOnly),/no indexable/);
});
...
test('EML source dispatch uploads normalized text and not attachment bytes',()=>{
  ...
  const built=plain(ksp.kspBuildFeatureFreezeAiSource(env,{sourceType:'Pitchbook',sourceId:'DOC-000001',row:c.pitchbookRows[0]},ksp.kspBuildAiMasterMaps(c.gpRows,c.optionRows)));
...
test('NotIndexed active source is eligible after Work 0008 deferral',()=>{
  ...
  const selected=plain(ksp.kspFfSelectAiWorkItems(c.meetingRows,c.pitchbookRows,'2026-08-16T00:00:00.000Z',ksp.kspNormalizeAiSettings(c.settings)));
  assert.equal(selected.length,1);
});

test('unchanged row is repaired when derived Document is missing',()=>{
  const c=baseContext({pitchbookRows:[]});const hash=ksp.kspAiHashTextFallback('Meeting body');Object.assign(c.meetingRows[0],{AI_Index_Status:'Pending',AI_Content_Hash:hash,AI_Document_Name:'fileSearchStores/store-1/documents/missing'});
  const env=createSyncEnvironment({context:c,documents:[]});const result=plain(ksp.kspRunFeatureFreezeAiSync(env));
...
test('PDF feature-freeze sync indexes binary source instead of deferring it',()=>{
  ...
  const result=plain(ksp.kspRunFeatureFreezeAiSync(env));
...
test('five search modes use one File Search tool contract',()=>{
  const definitions=plain(ksp.kspGetFeatureFreezeModeDefinitions());
  assert.deepEqual(definitions.map(x=>x.mode),['自由質問','要約','時系列','比較','面談準備']);
  for(const mode of definitions.map(x=>x.mode)){
    const request=plain(ksp.kspBuildFeatureFreezeInteractionRequest({modelId:'flash',storeName:'fileSearchStores/store-1',mode,questionOrInstruction:mode==='自由質問'?'Q':'',gpId:mode==='面談準備'?'GP-1':'',metadataFilter:'gp_id = "GP-1"'}));
...
test('mode validation requires question only for free question and GP for meeting prep',()=>{
  assert.throws(()=>ksp.kspValidateFeatureFreezeSearchInput(ksp.kspNormalizeFeatureFreezeSearchInput({mode:'自由質問'})),/質問/);
  assert.doesNotThrow(()=>ksp.kspValidateFeatureFreezeSearchInput(ksp.kspNormalizeFeatureFreezeSearchInput({mode:'要約'})));
  assert.throws(()=>ksp.kspValidateFeatureFreezeSearchInput(ksp.kspNormalizeFeatureFreezeSearchInput({mode:'面談準備'})),/GP/);
  assert.doesNotThrow(()=>ksp.kspValidateFeatureFreezeSearchInput(ksp.kspNormalizeFeatureFreezeSearchInput({mode:'面談準備',gpId:'GP-1'})));
});

test('mode prompts encode synthesis, chronology, comparison, and meeting prep guardrails',()=>{
  assert.match(ksp.kspBuildFeatureFreezePrompt({mode:'要約'}),/単純に並べない/);
  assert.match(ksp.kspBuildFeatureFreezePrompt({mode:'時系列'}),/変化と継続/);
  assert.match(ksp.kspBuildFeatureFreezePrompt({mode:'比較'}),/順位付け/);
  assert.match(ksp.kspBuildFeatureFreezePrompt({mode:'面談準備'}),/未解決論点/);
});
...
test('generic five-mode service audits mode without answer or chunks',()=>{
  ...
  const result=plain(ksp.kspRunFeatureFreezeKnowledgeSearch(env,{mode:'要約',questionOrInstruction:'risk focus'}));
  assert.equal(result.ok,true);assert.equal(result.mode,'要約');assert.equal(env._debug.audits[0].Search_Mode,'要約');assert.equal(env._debug.audits[0].Question_Or_Instruction,'risk focus');assert.equal(JSON.stringify(env._debug.audits[0]).includes('SECRET_ANSWER'),false);
});

test('feature-freeze diagnostics report six formats, five modes, and pending live qualification',()=>{
  const result=plain(ksp.kspGetFeatureFreezeDiagnostics(createSyncEnvironment()));
  assert.equal(result.featureFreezeCandidate,true);assert.equal(result.formats.length,6);assert.equal(result.modes.length,5);assert.equal(result.sharedRetrievalPath,'kspRunFeatureFreezeKnowledgeSearch');assert.equal(result.liveQualified,false);assert.equal(result.syncHandlerAvailable,true);assert.equal(Object.hasOwn(result,'apiKey'),false);
});
