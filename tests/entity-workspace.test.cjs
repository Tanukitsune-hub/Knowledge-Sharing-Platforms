...
test('GP mode separates direct and related activity, owns exact Pitchbooks, and reuses explicit edges', () => {
  ...
  assert.equal(result.summary.directMeetingCount, 2);
  assert.equal(result.summary.relatedMeetingCount, 2);
...
test('GP Workspace compatibility delegates to the shared Entity Workspace model', () => {
  ...
  assert.equal(result.summary.meetingTotal, 2);
  ...
  assert.match(source, /meetingScope: 'direct'/);
});
