  ...
  const env = {
    ...
    getSheetHeaders(id, sheet) {
      if (id === 'backend') {
        ...
        if (sheet === 'Pitchbook_Index') return ['Document_ID','Batch_ID','Date','GP_ID','Asset_Class_ID','Capital_Type_ID','Sequence_No','File_ID','File_URL','Original_Filename','Saved_Filename','Status','Created_At','Updated_At','Created_By','Updated_By','AI_Document_Name','AI_Index_Status','AI_Indexed_At','AI_Content_Hash','AI_Last_Error','Fund_Strategy','AI_Provider_State_JSON'];
