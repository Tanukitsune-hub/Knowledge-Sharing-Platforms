...
test('free question maps citations and audits only safe query metadata',()=>{
  ...
  const r=plain(ksp.kspRunFreeQuestion(env,{question:'What did KKR say?',gpId:'GP-1'}));
...
test('free question surfaces insufficient evidence when authoritative citations are absent',()=>{
  const env=createSyncEnvironment({queryResponse:{steps:[{type:'model_output',content:[{type:'text',text:'A possible answer',annotations:[]}]}]}});const r=plain(ksp.kspRunFreeQuestion(env,{question:'Q'}));assert.equal(r.ok,true);assert.equal(r.insufficientEvidence,true);assert.equal(r.citations.length,0);
});

test('audit and Actor failures are non-blocking for a successful query',()=>{
  const env=createSyncEnvironment({actorError:true,auditError:true,queryResponse:{steps:[{type:'model_output',content:[{type:'text',text:'Answer',annotations:[{type:'file_citation',source:'d',custom_metadata:[{key:'source_id',string_value:'MTG-000001'}]}]}]}]}});const r=plain(ksp.kspRunFreeQuestion(env,{question:'Q'}));assert.equal(r.ok,true);assert.ok(r.warnings.some(w=>w.code==='ACTOR_RESOLUTION_FAILED'));assert.ok(r.warnings.some(w=>w.code==='AUDIT_WRITE_FAILED'));
});

test('query error is returned and failure audit excludes answer content',()=>{
  const error=new Error('service down');error.code='AI_HTTP_503';const env=createSyncEnvironment({queryError:error});const r=plain(ksp.kspRunFreeQuestion(env,{question:'Q'}));assert.equal(r.ok,false);assert.equal(r.error.code,'AI_HTTP_503');assert.equal(env._debug.audits[0].Result,'Failure');
});

test('question/date/source validation rejects invalid requests',()=>{
  assert.throws(()=>ksp.kspValidateKnowledgeSearchInput(ksp.kspNormalizeKnowledgeSearchInput({question:''})),/質問/);
  assert.throws(()=>ksp.kspValidateKnowledgeSearchInput(ksp.kspNormalizeKnowledgeSearchInput({question:'Q',dateFrom:'2026-12-31',dateTo:'2026-01-01'})),/Date From/);
  assert.throws(()=>ksp.kspValidateKnowledgeSearchInput(ksp.kspNormalizeKnowledgeSearchInput({question:'Q',sourceType:'Other'})),/Source Type/);
});

test('bootstrap reports configured state and active search options',()=>{
  const env=createSyncEnvironment();const r=plain(ksp.kspGetKnowledgeSearchBootstrap(env));assert.equal(r.ok,true);assert.equal(r.configured,true);assert.deepEqual(r.implementedModes,['自由質問']);assert.equal(r.options.gps.length,1);
});

test('settings normalization and retry backoff are bounded',()=>{
  const s=plain(ksp.kspNormalizeAiSettings({AI_SYNC_BATCH_SIZE:'500',AI_RETRY_BASE_MINUTES:'15',AI_RETRY_MAX_MINUTES:'60'}));assert.equal(s.syncBatchSize,10);assert.equal(ksp.kspCalculateAiRetryAt('2026-08-16T00:00:00.000Z',5,s),'2026-08-16T01:00:00.000Z');
});
...
test('all new Apps Script and client files parse and contain required live contract tokens',()=>{
  ...
  for(const token of ['id="knowledge-form"','getKnowledgeSearchBootstrapData','askKnowledgeQuestion','Citation','ClientKnowledgeSearch'])assert.ok((page+'\n'+client).includes(token),token);
...
test('official-style JSON fixtures normalize to stable internal contracts',()=>{
  ...
  const store=plain(ksp.kspNormalizeFileSearchStore(JSON.parse(fs.readFileSync(path.join(root,'store.json'),'utf8'))));
  const operation=plain(ksp.kspNormalizeFileSearchOperation(JSON.parse(fs.readFileSync(path.join(root,'upload-operation.json'),'utf8'))));
  const interaction=plain(ksp.kspParseInteractionResponse(JSON.parse(fs.readFileSync(path.join(root,'interaction.json'),'utf8'))));
