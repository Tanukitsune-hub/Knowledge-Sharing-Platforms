...
test('Entity Workspace is integrated, read-only, and has bounded print markup', () => {
  ...
  assert.doesNotMatch(page, /id="entity-workspace-type"|面談先種別/);
  ...
  assert.doesNotMatch(client, /entityWorkspacePopulateTypes|el\('entity-workspace-type'\)/);
...
test('Entity Workspace client loads catalog, selected entity, exact drill, and print without writes', async () => {
  ...
  const runtime = executeClient(async (method, payload) => {
    ...
    if (!payload.entityKey) return { ok: true, entityTypes: [{ code: 'GP', label: 'GP / 運用会社', entityCount: 1 }, { code: 'LP_ASSET_OWNER', label: 'LP / Asset Owner', entityCount: 1 }], entityOptions: [{ entityKey: 'GP:CP-1', type: 'GP', id: 'CP-1', name: 'Synthetic GP', status: 'Active' }, { entityKey: 'LP_ASSET_OWNER:LP-1', type: 'LP_ASSET_OWNER', id: 'LP-1', name: 'Synthetic LP', status: 'Active' }] };
  ...
  assert.deepEqual(Array.from(runtime.node('entity-workspace-entity').options, option => option.value), ['GP:CP-1', 'LP_ASSET_OWNER:LP-1']);
  runtime.context.entityWorkspacePopulateEntities('LP_ASSET_OWNER:LP-1');
