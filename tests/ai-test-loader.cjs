...
function loadAi() {
  ...
  const stub = `
    ...
    function kspIsValidDateKey_(v){var m=/^(\\d{4})-(\\d{2})-(\\d{2})$/.exec(String(v||''));if(!m)return false;var d=new Date(Date.UTC(+m[1],+m[2]-1,+m[3]));return d.getUTCFullYear()===+m[1]&&d.getUTCMonth()===+m[2]-1&&d.getUTCDate()===+m[3];}
  ...
  for (const file of [
    '00_Core.gs','30_MeetingCore.gs','130_AiConstants.gs','131_AiFileSearchContracts.gs','132_AiKnowledgeContracts.gs','133_AiRetryContracts.gs','161_GeminiRestClient.gs',
