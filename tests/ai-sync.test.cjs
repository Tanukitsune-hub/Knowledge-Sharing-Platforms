...
test('first sync indexes Meeting and TXT source and writes Indexed state',()=>{
  const env=createSyncEnvironment();const r=plain(ksp.kspRunAiSync(env));
...
test('unchanged indexed revision makes no upload',()=>{
  const c=baseContext();const hash=ksp.kspAiHashTextFallback('Meeting body');
  ...
  c.pitchbookRows=[];
  const env=createSyncEnvironment({context:c});const r=plain(ksp.kspRunAiSync(env));
...
test('reconciles matching modeled document and deletes extras',()=>{
  const c=baseContext({pitchbookRows:[]});const hash=ksp.kspAiHashTextFallback('Meeting body');
  ...
  const r=plain(ksp.kspRunAiSync(env));assert.equal(r.reused,1);assert.deepEqual(env._debug.deleted,['fileSearchStores/store-1/documents/old']);assert.equal(env._debug.uploaded.length,0);
});
...
test('replacement deletes old document and indexes latest revision once',()=>{
  ...
  const r=plain(ksp.kspRunAiSync(env));assert.equal(r.indexed,1);assert.equal(env._debug.deleted.includes('fileSearchStores/store-1/documents/old'),true);assert.equal(env._debug.documents.filter(d=>d.customMetadata.source_id==='MTG-000001').length,1);
});
...
test('retryable sync failure records bounded backoff and clears stale AI reference',()=>{
  ...
  const env=createSyncEnvironment({context:c,uploadError:err});const r=plain(ksp.kspRunAiSync(env));
...
test('non-TXT source is made NotIndexed and old derived document is removed without retry loop',()=>{
  ...
  const env=createSyncEnvironment({context:c,documents:[{name:'fileSearchStores/store-1/documents/old',customMetadata:{source_id:'DOC-000001',content_hash:'old'}}]});const r=plain(ksp.kspRunAiSync(env));
...
test('Inactive cleanup removes derived documents and clears AI fields',()=>{
  ...
  const env=createSyncEnvironment({context:c,documents:[{name:'fileSearchStores/store-1/documents/old',customMetadata:{source_id:'MTG-000001',content_hash:'h'}}]});const r=plain(ksp.kspRunAiSync(env));
...
test('claimed source is skipped without mutation',()=>{
  const c=baseContext({pitchbookRows:[]});const env=createSyncEnvironment({context:c,claimDenied:'Meeting:MTG-000001'});const r=plain(ksp.kspRunAiSync(env));assert.equal(r.skippedClaims,1);assert.equal(env._debug.patches.length,0);
});

test('disabled sync returns without Store or source mutation',()=>{
  const c=baseContext();c.settings.AI_SYNC_ENABLED='false';const env=createSyncEnvironment({context:c});const r=plain(ksp.kspRunAiSync(env));assert.equal(r.syncEnabled,false);assert.equal(r.selected,0);assert.equal(env._debug.uploaded.length,0);
});
