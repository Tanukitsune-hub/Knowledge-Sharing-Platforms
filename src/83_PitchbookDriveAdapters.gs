function kspAttachPitchbookDriveAdapters(meetingEnvironment) {
  ...
  meetingEnvironment.createOrReusePitchbookFile = function (parentFolderId, row, bytes, mimeType) {
    var query = "'" + kspEscapeDriveQueryLiteral(parentFolderId) + "' in parents and trashed = false" +
      " and appProperties has { key='kspDocumentId' and value='" +
      kspEscapeDriveQueryLiteral(String(row.Document_ID)) + "' }";
    ...
    kspAssert(matches.length <= 1, 'DUPLICATE_PITCHBOOK_FILES',
