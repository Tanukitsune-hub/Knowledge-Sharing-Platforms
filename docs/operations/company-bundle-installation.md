...
No personal Google Drive template, Git, Node.js, terminal, `clasp`, source repository checkout, or manual creation of multiple script files is required.
...
If the release guide says `Drive API service required`:
...
Do not split the bundle into multiple files and do not edit generated sections.
...
The release guide lists the expected scopes. Stop if the authorization request contains an unexpected product or permission.
...
The sheet shows the next action in plain language. Normal installation does not require reading execution logs or JSON.
...
or open the Web App URL and follow the status link.
...
Share the Web App URL with the approved internal users. Users do not need the bundle, Apps Script editor, Backend Spreadsheet, or Audit Spreadsheet.
...
- record application/schema/bundle versions;
...
The installer should reuse stored resources, migrate append-only schema changes, and avoid duplicates. It stops rather than guessing when multiple ambiguous resources exist.
...
```text
replace the old code with the new bundle
-> run installKnowledgeShare again
-> review readiness
-> create/update the Web App version as instructed
```

No manual schema or resource editing should be required.
