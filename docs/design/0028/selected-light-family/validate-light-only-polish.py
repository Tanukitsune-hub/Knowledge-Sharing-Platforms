...
require("面談記録を登録。関連資料は親Meetingの中で扱う" in overview, "Meeting-only add surface missing")
require("独立した資料だけの登録route" in overview, "standalone file-only route boundary missing")

meeting_past = (ROOT / "05-past-records-meeting.html").read_text(encoding="utf-8")
for required in ("記録一覧", "面談", "関連資料 3件", "Meeting_Index.Related_Pitchbook_IDs", "DOC-000201", "DOC-000202", "DOC-000299", "PITCHBOOK_NOT_FOUND", "資料を追加", "削除（紐付け解除）", "親Meeting_ID = MTG-000101"):
    require(required in meeting_past, f"past Meeting relationship view missing: {required}")
for forbidden in ("記録種別", "データ受領", "資料→関連面談"):
    require(forbidden not in meeting_past, f"past Meeting view contains removed surface: {forbidden}")
...
for required in ("面談記録", "面談", "親Meetingを先に登録", "stable <code>Meeting_ID</code>", "関連資料（任意）", "親Meetingの作成に失敗した場合", "standalone Pitchbook route"):
    require(required in meeting, f"record-centric creation surface missing: {required}")
for forbidden in ("記録種別", "DATA_RECEIPT", "データ受領", "受領資料（必須）", "受領背景メモ", "親記録を先に登録"):
    require(forbidden not in meeting, f"Meeting-only creation surface contains removed branch: {forbidden}")
