...
`render-design.py`は現在のCODEX-10 artifactを再生成します。Canonical baseは`origin/main` `f2d965c39ed2a1259d07aaff7aca8d60e6b68a8e`、Production source SHAは`9fa668619a0b91fb60ed53f696363d3954cf709e`、visual baselineはCODEX-09 / PR #46です。
...
- `面談と資料の関連`を独立destinationから外し、`過去の記録`のMeeting detailへ関連資料を統合した。
- 関係の正本は`Meeting_Index.Related_Pitchbook_IDs`の明示Document IDのみ。GP一致による推定を行わない。
- Meeting detailはresolved / Inactive / unresolvedを区別し、関連資料の追加とunlinkを同じMeeting contextで扱う。
- 新規ファイルはvalid parent `Meeting_ID`を先に確定してから登録する。standalone Pitchbook registrationは作成しない。
- 関係の追加・削除は既存Meeting registration/edit contractに残し、unlinkは物理削除と区別する。
...
`page-manifest.json`はreview-only navigation referenceを含む12画面です。`source-controls.json`はcurrent sourceから抽出した197 controlです。`render-design.py`は同じHTML referenceを再生成し、`validate-light-only-polish.py`はLight-only境界、7 destinations、Meeting-centric relationship統合、source contract labels/tokensを検証します。
