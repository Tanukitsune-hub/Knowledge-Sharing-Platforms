...
`PRESENTATION_ONLY`は既存Light familyのlabel、grouping、order、summary表現です。`FRONTEND_BEHAVIOR`はrecord type selector、parent-first state、details、file actions等のclient動作を伴います。Backend semanticsはdesign-onlyで変更しません。
...
| 記録を追加 | `記録種別 = 面談 / データ受領`を先頭に置き、親record first、任意/必須file policyを同じsurfaceで示す | Parent-first orderingはFuture BUILD contract |
| 面談 record | Date、counterparty、classification、notes、optional related filesを階層化 | Form groupingはPRESENTATION_ONLY |
| データ受領 record | Date、source counterparty、classification、background memo、required files。meeting-only fieldは表示しない | RequirednessはFuture BUILD contract |
...
|---|---|
| Parent record | Existing stable `Meeting_ID`; future `Record_Type = MEETING | DATA_RECEIPT` candidate |
...
1366×768ではpage横overflowを0にします。Tableはpageではなく`.table-wrap`内に閉じます。760px未満ではrecord type, flow, file panels, action rowを意味順にstackします。
