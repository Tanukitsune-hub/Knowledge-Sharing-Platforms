...
- Information architecture: `面談 / 資料` subtabs are absent from both target surfaces; `記録種別` clearly offers `面談 / データ受領`.
- Parent-first flow: numbered sequence shows parent record save, stable `Meeting_ID`, then file registration and `Document_ID` relationship.
- File semantics: Meeting upload is optional; Data Receipt upload is required; related-file `削除（紐付け解除）` is visibly separate from physical deletion.
