...
| Meeting-only registration | Meeting fields required by source contract | 3-step flow card | 親Meeting登録 → `Meeting_ID`発行 → file registration | Future BUILD contract |
| 面談 record | Date / 面談先 / Asset required | Meeting fields + long text lower section | 関連資料は任意 | PRESENTATION_ONLY |
| 面談 related files | optional | 全幅drop + file table | 親Meeting保存後に資料を追加 | Existing file-level retry boundary |
| 過去の記録 filters | optional | date / counterparty row + details | Meeting一覧を検索 | FRONTEND_BEHAVIOR |
| 過去の記録 list | Meeting anchors | one Meeting table | 詳細を開く / 編集 | PRESENTATION_ONLY |
