# Design tokens — selected Light A1.9

PR #43のvisual tokensを変更せず、semantic tokenとして継続します。
...
- Sidebar: 236px desktop、8 destinationのflat list。activeはbackground/border/text + 3px left strip。赤をicon、button、alert、badgeへ展開しない。
...
- Analytics: chart/tableを横並びにし、同じseriesまたはbreakdownを視覚と数値で照合できるようにする。
- Chart: interiorはLight固定。Dark実装はこのdispatchでは行わない。

`#E1001F`の通常要素使用数は0をvalidationで確認します。
