...
| 2. 面談 record登録 | 記録を追加 → `記録種別 = 面談` | 面談情報、任意関連資料、親record先行を同じsurfaceで確認 | Parent `Meeting_ID` first、file-level retry | PASS |
| 3. データ受領登録 | 記録を追加 → `記録種別 = データ受領` | 受領元、分類、背景memo、必須資料を明示。面談-only fieldは隠す | Parent record作成失敗時はfile registration 0件 | PASS |
| 4. 過去の記録と関連資料 | 過去の記録 → record detail | 一つのrecord listからresolved / Inactive / unresolved関連資料を確認 | Explicit Document IDsのみ。削除はunlink | PASS |
...
- Consistency: Sidebar activeは1件。record type、related-file actions、checkbox、buttonにはgold/borderを使い、red stripを複製しない。
...
- Visibility: `データ受領`の背景memoと必須file、Meetingの任意fileを同じrecord creation surfaceで比較できる。
...
Backend変更riskはNONEです。Parent-first registration、file retry、unlink、record type mappingはfuture BUILD時にkeyboard/focus/runtime/persistenceを確認します。
