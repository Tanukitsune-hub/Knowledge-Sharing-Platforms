"""Source-independent acceptance assertions for the CODEX-09 design artifact."""
...
required_order = ['for="search-gp"','for="search-source"','for="search-from"','for="search-to"','id="all-period"','for="search-mode"','for="knowledge-visible-model"','for="search-question"']
assert [search.index(x) for x in required_order] == sorted(search.index(x) for x in required_order)
for text in ['面談記録・資料','面談記録のみ','資料のみ','2023-09-07','2026-09-07','AIモデル','詳細条件','2〜5件','面談準備の対象']:
...
print('PASS: CODEX-09 layout, source scope, 9-column fixture, protected preset and no-network boundaries')
