...
(() => {
  ...
  // 条件変更後の旧結果を、新しい条件の回答・書き出しと誤認させない。
  const invalidate=()=>{$('search-answer').hidden=true;$('export-answer').hidden=true;};
  $('search-demo').addEventListener('input',invalidate);
  $('search-demo').addEventListener('change',invalidate);
  ...
  $('search-demo').addEventListener('submit',e=>{e.preventDefault();$('search-answer').hidden=true;$('export-answer').hidden=true;const c=common();if(!valid(c))return;
    ...
    // 現行契約のMeeting専用条件を維持し、未対応の混在を成功表示しない。
    let sourceType=$('search-source').value;
    if(!sourceType&&(c.details.relatedGp||c.details.meetingType))sourceType='Meeting';
    if(sourceType!=='Meeting'&&Object.keys(c.details).some(k=>['team','followup','relatedGp','meetingType'].includes(k))){status('Meeting専用条件は面談記録のみで利用できます。情報ソースと条件を確認してください。');return;}
    const sourceLabel=sourceType==='Meeting'?'面談記録のみ':sourceType==='Pitchbook'?'資料のみ':'面談記録・資料';
    const target=mode.value==='compare'?selected.join(', '):c.entityKey||'すべて';
    const sampleSources=(sourceType!=='Pitchbook'?'<p data-result-source="Meeting">面談記録：MTG-SYNTHETIC-101 / '+esc(target)+' / 面談原本</p>':'')+(sourceType!=='Meeting'?'<p data-result-source="Pitchbook">有効な親との関連がある適格な資料：DOC-SYNTHETIC-201 / '+esc(target)+' / 概要資料.pdf</p>':'');
    $('search-answer').innerHTML='<h2>検索結果の架空例</h2><p>実検索・通信なし。future source metadata契約の表示例です。</p><p>対象：'+esc(target)+' / '+esc(sourceLabel)+'</p><p>送信scope例：'+esc(JSON.stringify(mode.value==='compare'?{selectedEntityKeys:selected,sourceType}:{entityKey:c.entityKey,sourceType}))+'</p>'+sampleSources+'<p>原資料と出典のidentityは実装時にauthoritative sourceで検証します。</p>';
