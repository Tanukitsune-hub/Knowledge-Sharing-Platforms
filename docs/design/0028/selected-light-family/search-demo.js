...
(() => {
  ...
  const source = byId('search-source'), all = byId('all-period'), fullExport = byId('full-export-run');
  ...
  model.addEventListener('change', () => {
    byId('search-status').textContent = '管理者が許可したAIモデルを使用します';
    byId('search-answer').hidden = true; byId('export-answer').hidden = true;
  });
  fullExport.addEventListener('click', () => {
    byId('search-answer').hidden = true;
    byId('export-answer').hidden = false;
    byId('search-status').textContent = '全文出力プレビュー：面談記録のみ / AIなし';
    fullExport.setAttribute('aria-pressed', 'true');
  });
  byId('search-clear').addEventListener('click', () => {
    byId('search-demo').reset(); draft = ''; previousMode = 'summary'; previousSource = '';
    source.disabled = false; fullExport.removeAttribute('aria-pressed'); ['search-from','search-to'].forEach(id => { byId(id).disabled = false; });
  ...
  byId('search-demo').addEventListener('submit', event => {
    ...
    fullExport.removeAttribute('aria-pressed');
    byId('search-answer').hidden = false; byId('export-answer').hidden = true;
