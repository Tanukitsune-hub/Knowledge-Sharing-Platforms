...
User decision after viewing the images: prefer the type with a left-side sidebar; users primarily operate on wide screens.
...
Next safe action: issue a fresh `0028-CODEX-04` only when proceeding with the corrected selected-Light composite/reference. That dispatch should use Product Design, fix the known visual-contract errors, and stop for review before producing the selected Dark design unless explicitly scoped otherwise.
